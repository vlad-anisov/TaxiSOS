...
class SendToContactsTileService : TileService() {
    
    companion object {
        private const val TAG = "SendToContactsTile"
        private const val TILE_STATE_PREFS = "send_to_contacts_tile_state"
    ...
    private fun startWithTile() {
        Log.d(TAG, "Включаем отправку контактам")
        ...
        if (MainActivity.isActive) {
            ...
            val broadcastIntent = Intent(ACTION_TILE_TOGGLE).apply {
                ...
                putExtra("tile_action", "toggle_send_to_contacts")
    ...
    private fun stopTile() {
        Log.d(TAG, "Отключаем отправку контактам")
    ...
    private fun updateTile() {
        ...
        qsTile?.let { tile ->
            ...
            tile.label = if (isActive) {
                getString(R.string.tile_send_to_contacts_active)
            } else {
                getString(R.string.tile_send_to_contacts)
            }
