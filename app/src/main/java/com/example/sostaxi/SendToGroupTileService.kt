...
class SendToGroupTileService : TileService() {
    
    companion object {
        private const val TAG = "SendToGroupTile"
        private const val TILE_STATE_PREFS = "send_to_group_tile_state"
    ...
    private fun startWithTile() {
        Log.d(TAG, "Включаем отправку в группу")
        ...
        if (MainActivity.isActive) {
            ...
            val broadcastIntent = Intent(ACTION_TILE_TOGGLE).apply {
                ...
                putExtra("tile_action", "toggle_send_to_group")
    ...
    private fun stopTile() {
        Log.d(TAG, "Отключаем отправку в группу")
    ...
    private fun updateTile() {
        ...
        qsTile?.let { tile ->
            ...
            tile.label = if (isActive) {
                getString(R.string.tile_send_to_group_active)
            } else {
                getString(R.string.tile_send_to_group)
            }
